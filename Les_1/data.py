...
from math import pi, cos, sin, sqrt, floor, ceil, exp
...
def digits(xs, ys, model=None):
    ...
    Keyword arguments:
    xs       -- 144-element lists of 4-bit pixel values
    ...
    for n, ax in enumerate(axs[0]):
        ...
        ax.imshow(paint, extent=(0.0, 1.0, 0.0, 1.0), vmin=0, vmax=15, cmap=plt.cm.binary)
        ax.set_aspect('equal', 'box')
        ax.axis('off')
...
def cloud(nominal, num=256, dim=2, bias=None, weights=None, noise=0.0, seed=None):
...
def zigzag(num=512, noise=0.1, seed=None):
    """Generate a dataset containing a strongly curved datacloud.

    Keyword arguments:
    num      -- number of instances (default 512)
    noise    -- the amount of noise to add (default 0.1)
    seed     -- a seed to initialise the random number generator (default random)

    Return values:
    xs       -- values of the attributes x1 and x2
    ys       -- parametric one-dimensional coordinate
    """
    # Seed the random number generator
    random.seed(seed)
    # Generate attribute data
    ys = [[random.random()*2.0-1.0] for n in range(num)]
    xs = [[sin(y[0]*4.0-1.0)*0.8+0.3, exp(y[0]*0.7)*2.0-2.5] for y in ys]
    # Add noise to the attributes
    xs = [[xs[n][d]+random.gauss(0.0, noise) for d in range(2)] for n in range(num)]
    # Return values
    return xs, ys
