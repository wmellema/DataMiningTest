...
class Neuron():
    ...
    def train(self, x, y, alpha=0.01):
        ...
        pre_activation1 = self.bias + alpha * sum(wi * xi for wi, xi in zip(self.weights, x))
        ...
        post_activation1 = self.act_func(pre_activation1)
        ...
        #Pas de formules toe
        self.bias = self.bias + alpha * gradient_loss * gradient_act
        for count,j in enumerate(self.weights):
            self.weights[count] = self.weights[count] + alpha * gradient_loss * gradient_act * x[count]
